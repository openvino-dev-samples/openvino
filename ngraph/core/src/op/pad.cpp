...
BWDCMP_RTTI_DEFINITION(op::v8::Pad);
...
op::v8::Pad::Pad(const Output<Node>& arg,
                 const Output<Node>& pads_begin,
                 const Output<Node>& pads_end,
                 const Output<Node>& arg_pad_value,
                 PadMode pad_mode)
...
CoordinateDiff op::v8::Pad::get_pads_begin() const {
    CoordinateDiff pads_begin_coord{};
    if (auto pads_begin_const = get_constant_from_source(input_value(1))) {
        pads_begin_coord = pads_begin_const->cast_vector<ptrdiff_t>();
    }
...
CoordinateDiff op::v8::Pad::get_pads_end() const {
    CoordinateDiff pads_end_coord{};
    if (auto pads_end_const = get_constant_from_source(input_value(2))) {
        pads_end_coord = pads_end_const->cast_vector<ptrdiff_t>();
    }
...
bool ngraph::op::v8::Pad::visit_attributes(AttributeVisitor& visitor) {
    NGRAPH_OP_SCOPE(v8_Pad_visit_attributes);
    visitor.on_attribute("pad_mode", m_pad_mode);
    return true;
}

void op::v8::Pad::validate_and_infer_types() {
    NGRAPH_OP_SCOPE(v8_Pad_validate_and_infer_types);
    element::Type result_et;
            ...
            if (arg_shape[i].is_static()) {
                ...
                if (i > 1) {
                    ...
                    NODE_VALIDATION_CHECK(this,
                                          ...
                                          "spatial axis.");
                    NODE_VALIDATION_CHECK(this,
                                          m_pad_mode != op::PadMode::CIRCULAR || arg_shape[i].get_length() >= 1,
                                          "CIRCULAR padding mode requires an input of dimension of "
                                          "at least 1 at each "
                                          "spatial axis.");
                }
            }
...
shared_ptr<Node> op::v8::Pad::clone_with_new_inputs(const OutputVector& new_args) const {
    NGRAPH_OP_SCOPE(v8_Pad_clone_with_new_inputs);
    check_new_args_count(this, new_args);
    if (get_input_size() == 4) {
        return make_shared<v8::Pad>(new_args.at(0), new_args.at(1), new_args.at(2), new_args.at(3), m_pad_mode);
    } else {
        return make_shared<v8::Pad>(new_args.at(0), new_args.at(1), new_args.at(2), m_pad_mode);
    }
}

bool op::v8::Pad::evaluate_pad(const HostTensorVector& outputs, const HostTensorVector& inputs) const {
    const auto& data = inputs[0];
    const auto elem_size = data->get_element_type().size();
...
bool op::v8::Pad::evaluate(const HostTensorVector& outputs, const HostTensorVector& inputs) const {
    NGRAPH_OP_SCOPE(v8_Pad_evaluate);
    return evaluate_pad(outputs, inputs);
}

bool op::v8::Pad::has_evaluate() const {
    NGRAPH_OP_SCOPE(v8_Pad_has_evaluate);
    return true;
}
