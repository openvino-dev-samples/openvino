...
NamedOutputs top_k(const NodeContext& node) {
    ...
    if (node.has_ng_input("K")) {
        ...
        k_node = std::make_shared<ngraph::opset6::Convert>(k_variable, ngraph::element::i32);
    }else {
        int32_t k_expected;
        ...
        k_node = ngraph::opset6::Constant::create(ngraph::element::i32, {}, {k_expected});
    }
    int64_t axis = -1;
    const element::Type& index_element_type = ngraph::element::i64;
    auto node_topk = std::make_shared<ngraph::opset6::TopK>(x, k_node, axis, "max", "value", index_element_type);
