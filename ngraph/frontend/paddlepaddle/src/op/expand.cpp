...
NamedOutputs expand(const NodeContext& node) {
    ...
    Output<Node> expand_times_expected_node;
    if (node.has_ng_input("ExpandTimes")) {
        expand_times_expected_node = node.get_ng_input("ExpandTimes");
        ...
        expand_times_expected_node = std::make_shared<ngraph::opset6::Concat>(node_vec, 0);
    } else {
        ...
        expand_times_expected_node = ngraph::opset6::Constant::create(ngraph::element::i32, {times_expected.size()}, times_expected);
    }

    return node.default_single_output_mapping(
        {std::make_shared<ngraph::opset6::Tile>(
            x,
            std::make_shared<ngraph::opset6::Convert>(expand_times_expected_node, element::i64))},
        {"Out"});
}
