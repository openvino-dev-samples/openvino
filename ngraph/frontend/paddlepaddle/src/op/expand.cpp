...
NamedOutputs expand(const NodeContext& node) {
    ...
    if (node.has_ng_input("ExpandTimes")) {
        ...
        if (node.has_attribute<std::vector<int32_t>>("expand_times")) {
            ...
            throw std::runtime_error("expand: no expand_times attribute");
        }

        times_expected_node = ngraph::opset6::Constant::create(ngraph::element::i32, {times_expected.size()}, times_expected);
    }
