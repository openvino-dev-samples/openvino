...
ov::pass::VectorizedMOE3GEMMTransposeWeights::VectorizedMOE3GEMMTransposeWeights() {
    ...
    // Match the 3-GEMM SwiGLU MoE pattern with transpose_b=false
    // (produced by PyTorch patchers using torch.bmm with pre-transposed weights)
    // and convert to transpose_b=true so that FuseVectorizedMOE3GEMM can match.

    auto experts_input = wrap_type<v1::Reshape>({any_input(), any_input()});
    auto tile = wrap_type<v0::Tile>({experts_input, any_input()});
    auto after_tile_reshape = wrap_type<v1::Reshape>({tile, any_input()});

    // First GEMM (gate) — transpose_b=false
    auto gate_matmul =
        wrap_type<v0::MatMul>({after_tile_reshape, any_input()}, {{"transpose_a", false}, {"transpose_b", false}});
    auto swish = wrap_type<ov::op::v4::Swish>({gate_matmul});

    // Second GEMM (up) — transpose_b=false
    auto up_matmul =
        wrap_type<v0::MatMul>({after_tile_reshape, any_input()}, {{"transpose_a", false}, {"transpose_b", false}});

    // SwiGLU: Swish(gate) * up
    auto swiglu = wrap_type<v1::Multiply>({swish, up_matmul});

    // Third GEMM (down) — transpose_b=false
    auto down_matmul =
        wrap_type<v0::MatMul>({swiglu, any_input()}, {{"transpose_a", false}, {"transpose_b", false}});
    auto end_reshape = wrap_type<v1::Reshape>({down_matmul, any_input()});

    // Routing: ScatterElementsUpdate → Transpose → Reshape → Unsqueeze
    auto router_topk_indices = any_input();
    auto scatter_elements_update =
        wrap_type<ov::op::v12::ScatterElementsUpdate>({any_input(), router_topk_indices, any_input(), any_input()});
    auto router_transpose = wrap_type<v1::Transpose>({scatter_elements_update, any_input()});
    ...
    matcher_pass_callback callback = [=](Matcher& m) {
        ...
        auto gate_mm = ov::as_type_ptr<v0::MatMul>(pm.at(gate_matmul).get_node_shared_ptr());
        auto up_mm = ov::as_type_ptr<v0::MatMul>(pm.at(up_matmul).get_node_shared_ptr());
        auto down_mm = ov::as_type_ptr<v0::MatMul>(pm.at(down_matmul).get_node_shared_ptr());
        ...
        if (!insert_transpose(gate_mm) || !insert_transpose(up_mm) || !insert_transpose(down_mm))
            return false;

        ov::copy_runtime_info(m.get_matched_nodes(), {gate_mm, up_mm, down_mm});
        return true;
    };
